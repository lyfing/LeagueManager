...
- (id)initWithPlayer:(Player *)player andTeam:(Team *)team
{
    if ( self = [super init] ) {
        self.player = player;
        self.team = team;
...
- (void)setViewControllerStyle:(LMViewControllerStyle)mViewControllerStyle
{
    ...
    if ( LMViewControllerStyleDisplay == mViewControllerStyle ) {
        ...
        rightItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemEdit target:self action:@selector(eidtAction:)];
    }
...
{
    if ( [self.firstNameTField.text length] == 0 || [self.lastNameTField.text length] == 0 || [self.emailTField.text length] == 0 ) {
        return;
    }
